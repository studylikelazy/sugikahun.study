    ...
    <main className="market-shell min-h-screen bg-[#081115] text-[#e9efea] lg:grid lg:grid-cols-[278px_minmax(0,1fr)]">
      ...
      <section className="min-w-0">
        ...
        <div className="px-5 py-6 sm:px-8 lg:px-10 lg:py-8">
          ...
          <AssetOverview cash={cash} marketValue={marketValue} equity={equity} pnl={positionPnL} positions={positions} />
...
function AssetOverview({ cash, marketValue, equity, pnl, positions }: { cash: number; marketValue: number; equity: number; pnl: number; positions: Array<{ asset?: Asset; quantity: number; avgCost: number; value: number; pnl: number }> }) {
  return <section className="mt-5 overflow-hidden rounded-2xl border border-[#4f9bff]/25 bg-[linear-gradient(100deg,rgba(79,155,255,.13),rgba(13,26,30,.92)_52%,rgba(245,196,81,.06))] shadow-[0_18px_36px_rgba(0,0,0,.12)]"><div className="grid gap-0 min-[760px]:grid-cols-[1.15fr_.85fr]"><div className="p-5 sm:p-6"><div className="flex items-center justify-between"><div><p className="eyebrow text-[#74b5ff]">MY MONEY / いま使えるお金</p><p className="mt-1 font-display text-lg font-semibold text-white">合計資産</p></div><Banknote size={22} className="text-[#74b5ff]" /></div><p className="mt-3 font-mono text-4xl font-medium tracking-[-.07em] text-white">{formatMoney(equity, 0)}</p><div className="mt-5 grid grid-cols-3 gap-2"><MoneyTile label="手元資金" value={formatMoney(cash, 0)} accent="blue" /><MoneyTile label="保有資産" value={formatMoney(marketValue, 0)} accent="gold" /><MoneyTile label="含み損益" value={`${pnl >= 0 ? "+" : ""}${formatMoney(pnl, 0)}`} accent={pnl >= 0 ? "green" : "red"} /></div></div><div className="border-t border-white/[.09] bg-black/[.12] p-5 min-[760px]:border-l min-[760px]:border-t-0 sm:p-6"><div className="flex items-center justify-between"><div><p className="eyebrow">PORTFOLIO / 保有中</p><p className="mt-1 text-sm text-[#c3d0cc]">いま持っている銘柄の価値</p></div><span className="rounded-full bg-[#f5c451]/[.12] px-2.5 py-1 font-mono text-[9px] text-[#f5c451]">{positions.length} HOLDING</span></div>{positions.length === 0 ? <div className="mt-4 rounded-lg border border-dashed border-white/[.14] px-4 py-4 text-sm leading-relaxed text-[#93a4a5]">まだ保有している銘柄はありません。気になる銘柄を選んで、少額から試してみよう。</div> : <div className="mt-4 space-y-2">{positions.slice(0, 3).map((position) => position.asset && <div key={position.asset.id} className="flex items-center justify-between rounded-lg bg-white/[.045] px-3 py-2.5"><div><p className="font-display text-sm font-semibold text-white">{position.asset.code}<span className="ml-2 font-mono text-[9px] font-normal text-[#849599]">{position.quantity} UNIT</span></p><p className="font-mono text-[10px] text-[#8d9c9f]">平均 {formatMoney(position.avgCost, position.asset.kind === "Crypto" ? 3 : 2)}</p></div><div className="text-right"><p className="font-mono text-sm text-[#f5c451]">{formatMoney(position.value, 0)}</p><p className={`font-mono text-[10px] ${position.pnl >= 0 ? "text-[#36d399]" : "text-[#ff7474]"}`}>{position.pnl >= 0 ? "+" : ""}{formatMoney(position.pnl, 0)}</p></div></div>)}</div>}</div></div></section>;
}

function MoneyTile({ label, value, accent }: { label: string; value: string; accent: "blue" | "gold" | "green" | "red" }) {
  const colors = { blue: "border-[#4f9bff]/28 bg-[#4f9bff]/[.08] text-[#74b5ff]", gold: "border-[#f5c451]/28 bg-[#f5c451]/[.08] text-[#f5c451]", green: "border-[#36d399]/28 bg-[#36d399]/[.08] text-[#36d399]", red: "border-[#ff7474]/28 bg-[#ff7474]/[.08] text-[#ff9292]" };
